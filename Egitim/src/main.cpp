...
void moveSnake()
{
    ...
    {
        snakeList[i]->setDirection(snakeList[i-1]->getDirectin());
    }
}
...
void addToSnakeTail()
{
    ...
    else
    {
        ...
        Square::DIRECTION dir = lastSquare->getDirectin();

        switch (dir)
        {
        case Square::DIRECTION::DIR_RIGHT:
            pos -= glm::vec3(length,0.0f,0.0f);
            break;
        case Square::DIRECTION::DIR_LEFT:
            pos += glm::vec3(length,0.0f,0.0f);
            break;
        case Square::DIRECTION::DIR_UP:
            pos -= glm::vec3(0.0f,length,0.0f);
            break;
        case Square::DIRECTION::DIR_DOWN:
            pos += glm::vec3(0.0f,length,0.0f);
            break;
        }
