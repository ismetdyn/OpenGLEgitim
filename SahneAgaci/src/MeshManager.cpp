...
Mesh* MeshManager::createCylinder( 
    float length, 
    float radius, 
    ModelDetail detail,
...
Mesh *MeshManager::createCone(
    float length, float radius, 
    ModelDetail detail, const 
    glm::vec4 &frontColor, 
    const glm::vec4 &middleColor)
{
    if(!m_VaoManager->getVao("coneColored"))
    {
        Vertex3ColorList vertices;
        IndexList indices;
        float halfRadius = radius/2.0f;
        ...
        for(int i=0; i<circleVertexCount;i++)
        {
            ...
            float angle = (float) i*angleIncrease;
            vertex.pos.x = glm::cos(glm::radians(angle))*halfRadius;
            vertex.pos.z = glm::sin(glm::radians(angle))*halfRadius;

            vertex.pos.y = -length/2.0f;
            vertex.color = frontColor;

            vertices[i] = vertex;

            vertices[i+circleVertexCount]       = vertex;
            vertices[i+circleVertexCount].color = middleColor;
        }

        Vertex3Color topVertex;
        topVertex.pos = glm::vec3(0.0f,length/2,0.0f);
        topVertex.color = middleColor;
        vertices.push_back(topVertex);

        int index = circleVertexCount;
        for(int i=0; i<circleVertexCount-1;i++)
        {
            //Ön Yüz 
            indices.push_back(0);
        ...
        indices.push_back(circleVertexCount*2-1);
        ...
        VertexArrayObject* vao = new VertexArrayObject();
        vao->build(vertices,indices);
        m_VaoManager->addNewObject("coneColored",vao);
    }

    Mesh* mesh = new Mesh();
    mesh->m_Vao = m_VaoManager->getVao("coneColored");
    mesh->m_IndexCount = m_VaoManager->getVao("coneColored")->getIndexCount();
    mesh->m_VertexCount = m_VaoManager->getVao("coneColored")->getIndexCount();
